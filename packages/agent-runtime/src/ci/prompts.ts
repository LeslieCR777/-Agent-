...
import type { Battlecard, CiStage, Competitor, CompetitorChangeRow, ComparisonMatrix, ResearchInsightRow } from '@contracts/types.js';
...
export function buildResearchPrompt(
  ...
  const planBlock = [
    '调研计划：',
    '1. 问题：财务、产品技术、市场竞争、组织人才四类事实分别是什么？',
    '2. 信息缺口：找出每类问题尚无来源支撑的部分。',
    '3. 来源优先级：官网/监管披露 > 权威媒体 > 社区；同一原始来源的转载不算独立交叉验证。',
    '4. 停止条件：四类问题均有来源，或已耗尽给定查询；不得用猜测填补缺口。',
  ].join('\n');
  ...
  return [
    ...
    planBlock,
    ``,
    `调研方法：对每个主题先推理再下结论——先列出关键证据（变化/搜索命中），再推断对${competitorOnly ? '市场竞争格局' : '我方'}的影响。每条结论必须能回溯到来源，禁止臆造。`,
    ``,
    `已检测到的变化：\n${changesText}`,
    `${knownBlock}`,
    `搜索结果：\n${searchText}${feedbackBlock}`,
    ``,
    `输出 JSON 数组，每个元素：{"topic":"调研主题","summary":"四到六句话的完整分析，依次说明证据依据、事实判断、趋势解读和对${competitorOnly ? '市场竞争格局' : '我方'}的业务影响","key_findings":["包含时间、对象、数据或事件的可核查发现"],"impact":"对${competitorOnly ? '市场竞争格局' : '我方'}的影响评估（两到三句话）","sources":[{"title":"来源标题","url":"URL"}],"confidence":0到1之间的数字}。`,
    `【详细度规则】每个主题必须给出 4-6 条互不重复的 key_findings；有数字、日期、产品名、客户名或地区时必须保留。summary 不得只是复述要点，必须说明证据之间的联系、变化趋势以及对产品、销售或市场策略的具体启示。`,
    `覆盖【恰好 4 个主题】，每个主题聚焦一个子问题（不要多也不要少）：`,
    `  1. 财务状况：近期融资/营收/估值信号？资金来源与用途？`,
    `  2. 产品与技术：新功能/技术栈/开源动态？${competitorOnly ? '对其他竞品的相对竞争力？' : '对我方产品定位的威胁或机会？'}`,
    `  3. 市场与竞争：定价策略/市场份额/客户口碑？目标客群重合度？`,
    `  4. 组织与人才：招聘规模/高管变动/扩张信号？反映何种战略意图？`,
    `【impact 规则】${competitorOnly ? '必须直接说明该变化对市场竞争格局的影响方向和依据；禁止用无证据的条件式假设。' : '必须直接判定对我方是"威胁"还是"机会"（二选一），并一句话说明依据；禁止用"若重合/若错位/可能"等条件式假设。'}`,
...
type ComparisonInsights = Record<string, ResearchInsightRow[]>;

export function buildComparePrompt(
  ourProfile: Config['ourProduct'] | undefined,
  competitor: Competitor,
  insights: ResearchInsightRow[],
  comparisonCompetitors: Competitor[] = [],
  comparisonInsights: ComparisonInsights = {}
): string {
  const peers = comparisonCompetitors.slice(0, 2);
  if (!ourProfile && peers.length === 2) {
    const [left, right] = peers;
    const insightText = peers
      .map((peer) => {
        const peerInsights = comparisonInsights[peer.id] ?? (peer.id === competitor.id ? insights : []);
        return `【${peer.name}】\n${peerInsights.map((i) => `- ${i.topic}: ${i.summary}（置信度 ${i.confidence}）`).join('\n') || '（暂无调研洞察）'}`;
      })
      .join('\n');
    return [
      `你是一名竞品对比分析师。只比较以下两个已选竞品：「${left.name}」和「${right.name}」，不要引入任何我方产品或其他产品。基于调研洞察，在 8 个维度上分别给两者打分（0-10）。`,
      ``,
      `竞品 1：${left.name}${left.website ? ` (${left.website})` : ''}`,
      `竞品 2：${right.name}${right.website ? ` (${right.website})` : ''}`,
      ``,
      `调研洞察：\n${insightText}`,
      ``,
      `8 个维度必须齐全：\n${DIMENSIONS.map((d, i) => `${i + 1}. ${d}`).join('\n')}`,
      ``,
      `【防臆造规则】两个竞品的产品类型、市场定位等，若未在给定信息或调研洞察中提供，禁止自行设定——标注为"信息未提供，无法评估该维度"并给中性分 5。`,
      `评分必须有依据（调研洞察/已提供信息），无依据的维度给中性分并注明"无依据"。`,
      ``,
      `输出 JSON：{"left_competitor":"${left.name}","right_competitor":"${right.name}","dimensions":[{"dimension":"维度名","left_score":0-10,"right_score":0-10,"notes":"一句话说明"}],"overall_assessment":"整体评估（3句话）"}。`,
      JSON_ONLY,
    ].join('\n');
  }

  const profile = ourProfile ?? { name: '（未配置）', website: '', positioning: '', targetMarket: '' };
  ...
  return [
    ...
    ``,
    `我方产品：${profile.name}${profile.website ? ` (${profile.website})` : ''}\n定位：${profile.positioning || '（未配置）'}\n目标市场：${profile.targetMarket || '（未配置）'}`,
    ``,
...
export function buildStagePrompt(
  ...
  switch (stage) {
    ...
    case 'research':
      return buildResearchPrompt(ctx.competitor, ctx.latestChanges, ctx.searchResults, ctx.feedback, ctx.competitorOnly);
    case 'compare':
      return buildComparePrompt(
        ctx.competitorOnly ? undefined : ctx.ourProfile,
        ctx.competitor,
        ctx.latestInsights,
        ctx.comparisonCompetitors,
        ctx.comparisonInsights,
      );
    case 'battlecard': {
      const matrix = ctx.matrix ?? { dimensions: [], overall_assessment: '（暂无对比矩阵）' };
      if (!ctx.ourProfile) throw new Error('our product profile required for battlecard');
