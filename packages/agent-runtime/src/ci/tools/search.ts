...
/**
 * 监测阶段专用搜索。生产模式强制走 SerpAPI，不静默回退到
 * DuckDuckGo/Bing 或模拟数据，避免采集来源与配置预期不一致。
 */
export async function monitorSearch(query: string, numResults = 8): Promise<SearchResult[]> {
  if (config.ciDemoMode) return stubSearch(query).slice(0, numResults);
  if (!config.serpApi.key) throw new Error('SERPAPI_KEY is required for monitor stage');
  return serpSearch(query, numResults, config.serpApi.engine);
}
