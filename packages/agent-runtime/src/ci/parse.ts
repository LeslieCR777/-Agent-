...
function parseJsonBlockLegacy<T = unknown>(text: string): T | null {
  const t = stripFences(text);
  // Try all bracket candidates so unmatched punctuation in prose cannot hide valid JSON.
...
/** Parse the first valid JSON object or array from model output. */
export function parseJsonBlock<T = unknown>(text: string): T | null {
  const primary = parseJsonBlockLegacy<T>(text);
  if (primary !== null) return primary;
  const t = stripFences(text);
  for (let start = 0; start < t.length; start++) {
    if (t[start] !== '{' && t[start] !== '[') continue;
    for (let end = start + 1; end < t.length; end++) {
      if (t[end] !== '}' && t[end] !== ']') continue;
      try {
        return JSON.parse(t.slice(start, end + 1)) as T;
      } catch {
        // Try the next closing bracket or the next opening candidate.
      }
    }
  }
  return null;
