...
for (const v of [config.apiKey, config.embeddingApiKey, config.smtp.pass, config.serpApi.key, config.anthropic.apiKey, config.deepseek.apiKey, config.mysql.password]) {
  if (v) secrets.add(v);
}
