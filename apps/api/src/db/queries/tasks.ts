...
import { TRANSITIONS, TERMINAL_STATUSES } from '@contracts/constants.js';
import { insertEvent } from './events.js';
import { enqueueOutbox } from './analysis.js';
  ...
  return withTransaction(async () => {
    ...
    if (type) {
      ...
      if (input.status === 'completed' || input.status === 'failed') {
        await enqueueOutbox('task', input.taskId, `task.${input.status}`, {
          task_id: input.taskId,
        });
      }
    }
