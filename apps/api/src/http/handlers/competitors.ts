...
import {
  ...
  updateCompetitor,
  deleteCompetitor,
} from '@api/db/queries/competitors.js';
import { requestLegacyPipeline } from '@api/db/queries/analysis.js';
...
export const competitorsHandlers = {
  ...
  async analyze(req: ApiRequest, res: ServerResponse): Promise<void> {
    const competitor = await getCompetitor(req.params!.id);
    if (!competitor) throw new HttpError(404, 'NOT_FOUND');
    const request_id = await requestLegacyPipeline(competitor.id, 'full', req.actor ?? 'user:api');
    sendJson(res, 202, { request_id });
  },
  ...
  async monitor(req: ApiRequest, res: ServerResponse): Promise<void> {
    const competitor = await getCompetitor(req.params!.id);
    if (!competitor) throw new HttpError(404, 'NOT_FOUND');
    const request_id = await requestLegacyPipeline(competitor.id, 'monitor', req.actor ?? 'user:api');
    sendJson(res, 202, { request_id });
  },
};
