...
import { apiAsAgent, api, ApiClientError } from '@runtime/client.js';
import { runAgent, prepareTaskDir } from '@runtime/runner.js';
import { Heartbeater } from './heartbeat.js';
import { isCiTask, executeCiStage } from '@runtime/ci/execute.js';
...
async function executeTask(agentId: string, task: { id: string; title: string; prompt: string; attachments: string | null; tags: string | null }): Promise<void> {
  ...
  if (isCiTask(tags)) {
    try {
      ...
      logger.error('worker', `CI task ${task.id.slice(0, 8)} failed: ${reason}`);
  ...
  if (result.exitCode === 0 && !result.timedOut) {
    ...
    const reason = result.timedOut
      ...
      : (result.errorOutput.trim() || `Claude API failed (${result.exitCode})`);
    await apiAsAgent(agentId, `/api/tasks/${task.id}/status`, {
      method: 'PATCH',
      body: { status: 'failed', error: reason },
    }).catch(() => {});
