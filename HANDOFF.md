...
> 更新于：2026-08-05
> 状态：**✅ 全部完成**。MySQL 迁移 + 强模型 + 多 agent 评审 + 使用说明页均已完成并验证（tsc 0 错误、41/41 测试通过、端到端全链跑通）。
...
| # | 升级 | 状态 | 说明 |
|---|---|---|---|
| 1 | **MySQL 存储**（全面迁移） | ✅ 完成 | 连接层 mysql2 连接池 + AsyncLocalStorage 事务，11 query 异步化，claim 用 FOR UPDATE SKIP LOCKED。41/41 测试通过 |
| 2 | **更强模型 + 多 agent 评审** | ✅ 完成 | 分析主阶段 `AGENT_MODEL`（默认 claude-opus-5），评审用 DeepSeek API（`src/ci/judge.ts`），3 judge 投票聚合 |
| 3 | **竞品情报更详细** | ✅ 完成 | 各 stage prompt 增强：severity 准则/monitor、思考链+impact/research、评分锚点/compare、竞品定位+市场空白/battlecard、3 视角评审/quality |
| 4 | **使用说明页** | ✅ 完成 | 看板新增「使用说明」Tab（快速开始/流水线原理/监控选择/配置/FAQ） |
