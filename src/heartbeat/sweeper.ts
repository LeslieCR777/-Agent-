...
import { getDb, withTransaction, nowIso } from '../db/index.js';
...
export async function runSweep(): Promise<void> {
  ...
  try {
    const now = nowIso();
    const staleAgents = findStaleAgents(config.heartbeatTimeoutMs, now);
    for (const agent of staleAgents) {
      recoverAgent(agent, now);
    }
  } catch (err) {
...
function recoverAgent(agent: Agent, now: string): void {
  withTransaction(() => {
    // 1. 标 offline
    setAgentOffline(agent.id);
    ...
    const tasks = getDb()
      ...
      .all(agent.id) as { id: string; assign_count: number; status: string }[];

    for (const task of tasks) {
      const canRetry = task.assign_count < config.maxAssignCount;
      insertEvent({
        task_id: task.id,
        agent_id: agent.id,
        type: 'task_stale',
        payload: {
          ...
          agent: agent.name,
          assign_count: task.assign_count,
          will_retry: canRetry,
        },
      });
      ...
      if (canRetry) {
        ...
        getDb()
          ...
          .run('assign limit exceeded (task repeatedly crashed workers)', now, task.id);
      }
    }
