import { nowIso, getPool} from '../index.js';
...
export async function registerAgent(input: {
  ...
  const [rows] = await getPool().execute(`SELECT * FROM agents WHERE id = ?`, [input.id]);
  const row = rows[0];
...
export async function getAgent(id: string): Promise<Agent | null> {
  const [rows] = await getPool().execute(`SELECT * FROM agents WHERE id = ?`, [id]);
  return rows[0] ?? null;
}

export async function listAgents(): Promise<Agent[]> {
  const [rows] = await getPool().execute(`SELECT * FROM agents ORDER BY created_at ASC`);
  return rows;
}
